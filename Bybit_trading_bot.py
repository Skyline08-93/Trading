...
triangle_cache = {}
orderbook_cache = {}
orderbook_last_updated = datetime.min.replace(tzinfo=timezone.utc)
...
async def update_orderbooks(symbols: list):
    ...
    async with cache_lock:
        ...
        if (now - orderbook_last_updated).total_seconds() < ORDERBOOK_CACHE_TTL:
            return
    ...
    async with trade_lock:
        try:
            ...
            while not recent_trades.empty():
                item = recent_trades.get_nowait()
                recent_list.append(item)
                recent_trades.task_done()
                
            recent_count = sum(1 for t in recent_list if (now - t).total_seconds() < 60)
            for item in recent_list:
                recent_trades.put_nowait(item)
            ...
            with open(LOG_FILE, "a") as f:
                f.write(f"{now},{base},{mid1},{mid2},{start_amount},{amount3},{profit}\n")
...
async def check_triangle(
    ...
    try:
        ...
        async with cache_lock:
            ...
            now = datetime.now(timezone.utc)
            if last_seen and (now - last_seen).total_seconds() < TRIANGLE_HOLD_TIME:
                return
        ...
        # Обновляем кэш
        async with cache_lock:
            triangle_cache[triangle_id] = now
...
async def main():
    ...
    while True:
        try:
            ...
            async with cache_lock:
                now = datetime.now(timezone.utc)
                expired = [k for k, v in triangle_cache.items() if (now - v).total_seconds() > 3600]
                for k in expired:
            ...
            while not recent_trades.empty():
                ...
                if (now - trade_time).total_seconds() < 300:
                    temp_queue.put_nowait(trade_time)
            recent_trades = temp_queue
...
if __name__ == '__main__':
    try:
        ...
        asyncio.run(send_telegram_message("⏹ Бот остановлен пользователем"))
    finally:
